...
df=pd.read_csv(r'D:\all capstone\Singapore Flat Resale\final.csv')
df=df.drop(['Unnamed: 0'],axis=1) 

tab1,tab2,tab3,tab4=st.tabs(['HOME','PREDICTION','INSIGHTS','MY PROFILE'])
...
with tab2:
    ...
    if st.button('Predict Price'):
            ...
            def load_data():
                # Load data
                data = pd.read_csv(r'D:\all capstone\Singapore Flat Resale\final.csv')
            ...
            st.success(f" :rainbow[Resale Price:] :red[$] :green[{predicted_price}]")         
            

with tab3:

        st.markdown('### :violet[Key Insights...]')
...
with tab4:
    ...
    if button:
        st.success("**Thank you for utilizing this platform. I hope you have received the predicted price for your flat resale!❤️**")
